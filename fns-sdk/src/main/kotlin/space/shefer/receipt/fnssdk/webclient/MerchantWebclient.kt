...
class MerchantWebclient {
    ...
    fun getMerchantFromPrimaInformOrNone(inn: String): MerchantSuggestion? {
        ...
        responseEntity.body?.suggestions?.filter { it.data.inn == inn }?.map { suggestion ->
            mapOf("inn" to suggestion.data.inn,
                    "address" to suggestion.data.address, "name" to suggestion.value)
        }
